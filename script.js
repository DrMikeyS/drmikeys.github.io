...
getWeatherBtn.addEventListener('click', () => {
    ...
    fetchHistoricWeather(selectedCityLatitude, selectedCityLongitude, formattedAPIStartDate, formattedAPIEndDate);
});
...
// Function to fetch historic weather data from the API and render charts
async function fetchHistoricWeather(latitude, longitude, startDate, endDate) {
    const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${latitude}&longitude=${longitude}&start_date=${startDate}&end_date=${endDate}&daily=temperature_2m_max,rain_sum`;

    try {
        const response = await fetch(url);
        const data = await response.json();
        console.log('Historic weather data:', data);

        // Extract data for charts
        const dates = data.daily.time;
        const maxTemperatures = data.daily.temperature_2m_max;
        const rainfalls = data.daily.rain_sum;

        // Render temperature chart
        renderTemperatureChart(dates, maxTemperatures);

        // Render rainfall chart
        renderRainfallChart(dates, rainfalls);
    } catch (error) {
...
function renderTemperatureChart(dates, temperatures) {
...
// Function to render rainfall chart
function renderRainfallChart(dates, rainfalls) {
